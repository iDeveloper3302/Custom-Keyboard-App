...
let metrics: [String:Double] = [
    "topBanner": 0 //originally 30 to hold the top demo prediction bar
]
...
class KeyboardViewController: UIInputViewController {
    ...
    var emojiView: CustomCollectionView?
    var isEmojiHidden: Bool = true
    ...
    */
    ...
    @IBAction func toggleSettings() {
        ...
        if self.emojiView == nil {
            if let aCustomview = self.setupCollectionView() {
                ...
                self.emojiView = aCustomview
            }
        }
        
        if let emojiView = self.emojiView {
            emojiView.hidden = !isEmojiHidden
            self.forwardingView.hidden = isEmojiHidden
            self.forwardingView.userInteractionEnabled = !isEmojiHidden
            
        }
        isEmojiHidden = !isEmojiHidden
    }
    ...
    func setupCollectionView() -> CustomCollectionView? {
        
        let frame = UIScreen.mainScreen().bounds
        let customCollectionView: CustomCollectionView = CustomCollectionView(frame: frame)
        customCollectionView.delegate = self
        customCollectionView.imageSource = ImageSourceList
        customCollectionView.translatesAutoresizingMaskIntoConstraints = false
        self.view.addSubview(customCollectionView)
        
        let topConstraint = NSLayoutConstraint(item: customCollectionView, attribute: .Top, relatedBy: .Equal, toItem: view, attribute: .Top, multiplier: 1.0, constant: 0)
        let bottomConstraint = NSLayoutConstraint(item: customCollectionView, attribute: .Bottom, relatedBy: .Equal, toItem: view, attribute: .Bottom, multiplier: 1.0, constant: 0)
        let leftConstraint = NSLayoutConstraint(item: customCollectionView, attribute: .Left, relatedBy: .Equal, toItem: view, attribute: .Left, multiplier: 1.0, constant: 0)
        let rightConstraint = NSLayoutConstraint(item: customCollectionView, attribute: .Right, relatedBy: .Equal, toItem: view, attribute: .Right, multiplier: 1.0, constant: 0)
	...
	func getKeyboardType()
	{
        ...
        proxy.autocorrectionType == UITextAutocorrectionType.Yes
        proxy.spellCheckingType == UITextSpellCheckingType.Yes
		...
		else if(proxy.keyboardType == UIKeyboardType.Default)
        {
			...
			if(proxy.autocorrectionType == UITextAutocorrectionType.Yes)
			{
				key_type = true
...
extension KeyboardViewController: CustomCollectionViewDelegate {
    func didSelectItem(item: EmojiItem, sender: CustomCollectionView) {
        let string = item.link + "-1"
        print(string)
